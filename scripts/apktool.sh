...
DO_DECOMPILE()
{
    ...
    if [[ "$APK_PATH" == *rro_*.apk ]]; then
        apktool d -q -b $FORCE -o "$APKTOOL_DIR$OUT_DIR" -p "$FRAMEWORK_DIR" -s "$APK_PATH"
    else
        apktool d -q -b $FORCE -o "$APKTOOL_DIR$OUT_DIR" -p "$FRAMEWORK_DIR" -r -s "$APK_PATH"
    fi
...
DO_RECOMPILE()
{
    ...
    apktool b -q -p "$FRAMEWORK_DIR" -srp "$APKTOOL_DIR$IN_DIR"
